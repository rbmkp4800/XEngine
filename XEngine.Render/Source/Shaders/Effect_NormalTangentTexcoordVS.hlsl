...
VSOutput main(VSInput input)
{
	...
	float3 worldSpaceTangent = mul(input.tangent, (float3x3) transform);
	...
	output.tangent = normalize(mul((float3x3) cameraTransform.view, worldSpaceTangent));
	output.bitangent = normalize(mul((float3x3) cameraTransform.view, worldSpaceBitangent));
